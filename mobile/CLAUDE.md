...
Bottom-tab navigator (`src/navigation/RootNavigator.tsx`) with two tabs: `Home` (HomeScreen) and `Profile` (ProfileStack). `RootTabParamList` is the root typed contract. `ProfileStack` is a native stack inside the Profile tab hosting `ProfileMain`, `Signin`, and `Signup` screens; `ProfileStackParamList` / `ProfileNavigationProp` are the typed contracts for that stack.
