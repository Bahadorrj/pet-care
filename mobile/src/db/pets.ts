...
export function insertPet(
  ...
  db.runSync(
    `INSERT INTO pets (id, name, species, gender, photo_uri, notes, breed, weight_value, weight_unit, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      id,
      data.name,
...
export function updatePet(
  ...
  const now = new Date().toISOString();
  db.runSync(
    `UPDATE pets SET name = ?, species = ?, gender = ?, photo_uri = ?, notes = ?, breed = ?, weight_value = ?, weight_unit = ?, updated_at = ?
     WHERE id = ?`,
    [
      data.name,
      data.species,
