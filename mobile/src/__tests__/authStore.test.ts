 ...
 *
 * Verifies:
 * - login sets state and persists token/email to SecureStore
 * - logout clears state and removes entries from SecureStore
 * - hydrate with a stored token → isAuthenticated=true, hasHydrated=true
...
describe('authStore – login', () => {
  test('sets token, email, isAuthenticated=true and calls setItemAsync', async () => {
    const { useAuthStore } = await loadFreshStore();

    await useAuthStore.getState().login('tok123', 'user@example.com');
...
describe('authStore – logout', () => {
  test('clears state and calls deleteItemAsync for token and email', async () => {
    const { useAuthStore } = await loadFreshStore();
...
describe('authStore – hydrate', () => {
  test('with stored token → isAuthenticated=true and hasHydrated=true', async () => {
