...
describe("ProfileScreen – logged out", () => {
  test("renders the signin prompt", async () => {
    ...
    expect(screen.getByText(i18n.t("profile.signin_prompt"))).toBeTruthy();
  });

  test("renders the sign-in/sign-up button", async () => {
    await render(<ProfileScreen />);
    expect(screen.getByText(i18n.t("home.signin_signup"))).toBeTruthy();
  });
...
describe("ProfileScreen – logged in", () => {
  ...
  test("renders the logout button", async () => {
    await render(<ProfileScreen />);
    expect(screen.getByText(i18n.t("profile.logout"))).toBeTruthy();
  });
  ...
  test("pressing the logout button calls store logout()", async () => {
    ...
    await render(<ProfileScreen />);
    fireEvent.press(screen.getByText(i18n.t("profile.logout")));
    expect(mockLogout).toHaveBeenCalledTimes(1);
  });

  test("renders the change username button", async () => {
    await render(<ProfileScreen />);
    expect(screen.getByText(i18n.t("profile.change_username"))).toBeTruthy();
  });

  test("pressing the change username button navigates to ChangeUsername", async () => {
    await render(<ProfileScreen />);
    fireEvent.press(screen.getByText(i18n.t("profile.change_username")));
